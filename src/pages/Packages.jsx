...
const Packages = () => {
  const columns = [
    {
      ...
      render: (text, record) => (
        <div className="flex items-center gap-2">
          ...
          {record.popular && (
            <Tag icon={<StarFilled />} color="gold" className="font-semibold">
              الأكثر طلباً
            </Tag>
          )}
        </div>
    ...
    },
    {
      ...
      render: (text) => (
        <Tag
          color="green"
          className="px-3 py-1 text-xl rounded-full font-bold"
          dir="rtl"
        >{`${text}`}</Tag>
      ),
    },
    {
      ...
      key: "speed",
      render: (text) => (
        <span className="font-bold text-lg gradient-text">{text}</span>
      ),
    },
  ...
  return (
    <div className="py-20 min-h-screen relative overflow-hidden">
      ...
      <div
        className="absolute bottom-10 left-20 w-96 h-96 bg-purple-300 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float"
      ...
      <div className="px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="hidden md:block glass-effect rounded-3xl shadow-2xl overflow-hidden animate-fade-in">
          <Table
            ...
            rowClassName={(record) =>
              record.popular ? "bg-gradient-to-l from-yellow-50 to-white" : ""
            }
          />
        </div>
